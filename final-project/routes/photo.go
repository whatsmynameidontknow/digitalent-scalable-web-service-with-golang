...
func InitPhotoRoutes(r *http.ServeMux, db *sql.DB, logger *slog.Logger) {
	...
	r.Handle("POST /photos", middleware.AllowedContentType(middleware.Auth(http.HandlerFunc(controller.Create))))
	r.Handle("GET /photos", middleware.Auth(http.HandlerFunc(controller.GetAll)))
	r.Handle("PUT /photos/{photoID}", middleware.AllowedContentType(middleware.Auth(http.HandlerFunc(controller.Update))))
